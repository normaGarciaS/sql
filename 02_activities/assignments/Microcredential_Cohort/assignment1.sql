...
-- I need to review the variables-- 
--END QUERY
...
select 
vendor_id, count(*) as times_rented
...
from customer_purchases as cp
	left join customer as c on c.customer_id = cp.customer_id
...
SELECT 
    customer_id, 
    SUM(quantity * cost_to_customer_per_qty) AS total_spent_042022
FROM customer_purchases
GROUP BY customer_id,  STRFTIME('%m', market_date), STRFTIME('%Y', market_date)
having  STRFTIME('%m', market_date)='04' and STRFTIME('%Y', market_date)= '2022';
